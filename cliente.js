...
// Aggiungi queste funzioni in cliente.js--- aggiunte ora per logica di ricerca

function applicaFiltriCliente() {
    const testo = document.getElementById('cliRicerca').value.toLowerCase().trim();
    const dataInizio = document.getElementById('cliDataInizio').value;
    const dataFine = document.getElementById('cliDataFine').value;
    const stato = document.getElementById('cliStato').value;

    const filtrati = ordiniCaricatiLocali.filter(ordine => { // Usa la variabile che avevamo già definito
        let matchTesto = true;
        let matchData = true;
        let matchStato = true;

        // 1. Testo (Riferimento o Numero Ordine)
        if (testo) {
    ...
    renderOrdiniCliente(filtrati);
}

// Separa il rendering dal caricamento (Sostituisci la vecchia logica interna di caricaMieiOrdini)
function renderOrdiniCliente(ordiniDaMostrare) {
    const container = document.getElementById('ordiniListaCliente');
    if (ordiniDaMostrare.length === 0) {
        container.innerHTML = '<p>Nessun ordine trovato con i filtri selezionati.</p>';
        return;
    }
    ...
    ordiniDaMostrare.forEach(ordine => {
        ...
        html += `
            <tr>
                ...
                <td><span class="stato-ordine stato-${ordine.stato.replace(/\s/g, '-')}">${ordine.stato}</span></td>
                <td>
                    <button onclick="apriDettagliOrdine('${ordine.id}')" class="btn-primary" style="padding: 5px 10px;">Vedi Dettagli</button>
                </td>
            </tr>`;
    });
    html += '</tbody></table></div>';
